...
-----------------------------------------------------------------------------------------------
--  BY VictorCli       ---      End KEKO JSON              ---          BY VictorCli       --
-----------------------------------------------------------------------------------------------
...
local function send(chat_id, reply_to_message_id, disable_notification, text, disable_web_page_preview, parse_mode)
  ...
  {
  {text = ''..(channel_ts or "🃏┇TshAkE TEAM")..'', url=''..(channel_user_ts or 't.me/VictorCli')..''},
  },
  }
  local zain = "https://api.telegram.org/bot" ..token.. '/sendMessage?chat_id=' .. chat_id
  if reply_to_message_id ~= 0 then
  zain = zain .. '&reply_to_message_id=' .. reply_to_message_id/2097152/0.5 -- جميع الحقوق محفوظه لفريق فكتور لايمكنك نسخ او استخدام هذه السطر بدون موافقه الفريق
  end
...
local files_tshake = database:smembers("files"..bot_id)
local zain = io.popen('cd files_tshake && ls'):read("*all")
local files_tshake2 = ''
...
local photo_zain = download_to_file('https://api.telegram.org/file/bot'..token..'/'..zain6.result.file_path, 'files_tshake/'..data.message_.content_.document_.file_name_)
...
local kt = tshakeee.zain_tshake(data)
if kt == 'end' then
return false
end
end
...
local nazain = redis:get('nazain'..bot_id)
  ...
  sendContact(msg.chat_id_, msg.id_, 0, 1, nil, (nzain or 9647707641864), (nazain or "TshAke TEAM"), "", bot_id)
end
 end
  for k,v in pairs(sudo_users) do
...
local zain222 = 'https://tshake.tk/TshakeApi/ch.php?id='..msg.sender_user_id_..''
...
if text:match("^source$") or text:match("^اصدار$") or text:match("^الاصدار$") or text:match("^السورس$") or text:match("^سورس$") then
   ...
   local text =  [[
👋┇اهلا بك في سورس فكتور 🃏ֆ

🌐┇<strong>Victor TEAM</strong>

◀┇<a href="https://telegram.me/VictorCli">قناة البوت، 🃏" </a>
◀┇<a href="https://telegram.me/VictorCli">شروحات خاصة بالبوت، 🃏" </a>

🔎┇<a Devloper 1="https://telegram.me/LGLOBLA">ZAIN ALABDEEN </a>

🔎┇<a Devloper 2="https://telegram.me/ArslanMahDi"> Arslan MahDi </a>
